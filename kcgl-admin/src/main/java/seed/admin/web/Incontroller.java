...
import seed.sys.service.FoodInService;
import seed.sys.service.FoodService;

@Controller
@RequestMapping("sys/in")
public class Incontroller extends AbstractBaseCURDController<FoodIn,Long>  {
	...
	@Override
	public FoodInService getSimpleCurdService() {
		return (FoodInService)super.getSimpleCurdService();
	}
	...
	@RequestMapping("foodIn")
	public String foodIn(FoodIn m,String foodCode,Model model) {
		...
		model.addAttribute("response",new SuccessResponse("入库成功！"));
	...
	public Response list(Integer start, Integer length, String value,String columnname) {
		...
		int pageSize = length;
		Page<FoodIn> m = simpleCurdService.findAll(pageNumber, pageSize, value,columnname);
		return new DataTableResponse<FoodIn>( m.getContent(),(int) m.getTotalElements() );
	}
